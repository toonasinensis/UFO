...
DEFAULT_RUN = ROOT / "runs/add_sim_delay_20260730_101320"
DEFAULT_MOTION = ROOT / "humanoidverse/data/roban/named_roban_lafan/aiming1_subject1.npz"
DEFAULT_SCENE = ROOT / "humanoidverse/data/robots/biped_s17/xml/scene_fixedheadjoint.xml"
...
def _max_local_observation(
    ...
    """NumPy equivalent of compute_humanoid_observations_max (358 dims)."""
    root_pos = body_pos[0]
...
def run(args: argparse.Namespace) -> None:
    ...
    xml_path = (args.xml or Path(meta["xml_path"])).expanduser().resolve()
    scene_path = args.scene.expanduser().resolve()
    config_path = (args.robot_config or Path(meta["robot_config_path"])).expanduser().resolve()
    model = _load_scene(
        scene_path,
    ...
    qpos_frames, motion_fps = _load_motion(args.motion.expanduser().resolve(), joint_names)
    ...
    kp = np.asarray(control_metadata["p_gains"], dtype=np.float64)
    kd = np.asarray(control_metadata["d_gains"], dtype=np.float64)
    ...
    # Match the passive joint dynamics and ground contact used by LEJULAB's
    # BFM MuJoCo scene adapter. Apply these overrides last so the robot YAML's
    # friction values do not replace the LEJULAB settings.
    floor_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, "floor")
    if floor_id < 0:
        raise ValueError("MuJoCo scene has no floor geom")
    model.geom_friction[floor_id] = np.array([0.4, 0.005, 0.0001], dtype=np.float64)
    model.dof_damping[joint_dof_adr] = 0.0
    model.dof_frictionloss[joint_dof_adr] = 0.02
